...
fn main() -> io::Result<()> {
    ...
    writeln!(writer, "\"Generated by max-clique-gen\"")?;
    writeln!(writer)?;

    writeln!(writer, "\"Property: if two vertices are in a clique, then they should be connected.\"")?;
    writeln!(writer, "\"Property: if two vertices are not connected, they cannot be in the same clique.\"")?;
    writeln!(writer)?;
    ...
    for complement in &edges_complement {
        writeln!(
            writer,
            "  -({} & {}) &",
            format!("v_{}", complement.0),
            format!("v_{}", complement.1)
        )?;
    }
